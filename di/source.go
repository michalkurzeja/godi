...
// wiringMu guards wiringPackages. Registration usually happens once, from an
// init. Reading happens when a definition is asked where it came from. Neither
// is hot, so the lock costs nothing.
...
// declaredAt is where a function is declared.
//
// A factory godi synthesised has no source of the user's to point at: a value
// registered as a service is wrapped in a closure of godi's own. Those come back
// empty, rather than pointing into godi. So does a method value, whose wrapper
